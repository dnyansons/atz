...
class Shipping_Discount extends CI_Controller {
    ...
    public function index() {
        $this->load->view("admin/shipping_discount/list");
    }
    ...
    public function addShippingOffer() {
        ...
        if (strtotime($this->input->post('valid_from')) > strtotime($this->input->post('valid_to'))) {
            ...
            redirect("admin/shipping_discount", "refresh");
            exit;
        }
        if ($this->form_validation->run() === false) {
            ...
            $this->load->view("admin/shipping_discount/create", $data);
        } else {
            ...
            if ($tot_aval > 0) {
                ...
                $s3FilePath = $this->awsupload->upload('myfile', 'uploads/offer', 'image');
                if ($s3FilePath == false) {
                    //error
                    $this->session->set_flashdata('message', "<div class='alert alert-danger alert-dismissible'>
                            <a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>
                            <strong></strong>File not uploaded.</div>");
