...
class Bulkimport extends CI_Controller {
    ...
    public function index() {
        $this->load->view("admin/bulkimport/list");
    }
    ...
    public function fetch_bulkimport() {
        ...
        if (!empty($vendors)) {
            ...
            foreach ($vendors as $vdr) {
                ...
                $nestedData['upload_by'] = $vdr->upload_by;
    ...
    public function import() 
    {
        ...
        if ($this->form_validation->run() === false) {
            $this->load->view("admin/bulkimport/import");
        } else {
            //count Existing File
...
//                print_r($failed);
                ...
                $dat['user_id'] = $this->session->userdata("admin_id");
                $dat['upload_by'] = 'Admin - ' . $this->session->userdata("admin_username");
                $dat['uploaded_by'] = 'Admin';
    ...
    private function validate_data($row)
    {
        ...
        if(empty($row[0]) || empty($row[1]) || empty($row[2]) 
            ...
            || empty($row[6]) || empty($row[17]) || empty($row[18]) 
            || empty($row[19]) || empty($row[20]) || empty($row[21]) 
