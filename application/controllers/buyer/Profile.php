...
class Profile extends CI_Controller 
{
    ...
    function company_detail() 
    {
        ...
        redirect(base_url() . "buyer/profile");
    }
            ...
            if ($this->input->post('show_production_process') == 'YES') {
                ...
                for ($i = 0; $i < $count_row; $i++) {
                    if (isset($_FILES['process_picture']['name'][$i]) && !empty($_FILES['process_picture']['name'][$i])) {
                        ...
                        $_FILES['userFile']['name'] = $_FILES['process_picture']['name'][$i];
                        $_FILES['userFile']['type'] = $_FILES['process_picture']['type'][$i];
                        $_FILES['userFile']['tmp_name'] = $_FILES['process_picture']['tmp_name'][$i];
                        $_FILES['userFile']['error'] = $_FILES['process_picture']['error'][$i];
                        $_FILES['userFile']['size'] = $_FILES['process_picture']['size'][$i];
                        $config['upload_path'] = './uploads/supplier/';
                        $config['allowed_types'] = 'gif|jpg|png|jpeg|bmp';
                        $config['max_size'] = 999999999999;
                        $new_name = 'supp_' . $_FILES['userFile']['name'];
                        ...
                        $dat['process_picture'] = $image_name[$i];
                    }
