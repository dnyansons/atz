...
class Product_model extends CI_Model {
    ...
    var $selectOffer = 'oz.offer_id,offer_type,discount_value as offer_discount_value,oz.status as offer_status, '
            ...
            . 'valid_from, time_from,valid_to, time_to, ';
    /*
    ...
    public function get_datatables($from, $to, $seller2 = 0, $category, $name, $publish_status, $bulk) {
    ...
    public function count_filtered($from, $to, $seller2 = 0, $category, $name, $publish_status, $bulk) {
    ...
    private function _get_datatables_query($from, $to, $seller2 = 0, $category, $name, $publish_status, $bulk) {
        //echo "seller ".$seller2;
        $this->db
                ->select($this->select)
                ->from($this->_table . " P")
    ...
    function getProductDetails($products_id) { //api
        ...
        $this->db->select($this->selectOffer . ' track_inventory,low_stock_qty,notifyme,p.is_product_returnable,p.product_return_days,p.discount_percentage,p.publish_status, p.available_quantity, p.id, p.name,p.keywords,p.category as category_id,ctd.categories_name, p.description,p.weight,p.width,p.height,p.length,p.hike_percentage,p.provide_order_at_buyer_place,p.price_type,seller,sc.company_name,address1,main_products,other_products,year_of_register,no_of_employee,office_size,company_url,registration_state,location_country as country_name,currency as currency_name,comp_operational_addr,comp_operational_city,comp_operational_state,comp_operational_region,comp_operational_zip_code,ct.name business_type,users.user_package, p.meta_description,p.meta_title,p.surl');
        $this->db->from('product_details p');
        ...
        if ($result) {
            ...
            $i=0;
            foreach($spec_arr as $key=>$spec){
                if($i == 0){
                    $j=0;
                    foreach($spec as $sp){
                        $spec_arr[$key][$j]["type1"] = "Primary";
                        $j++;
                    }
                    ...
                    $j=0;
                    foreach($spec as $sp){
                        $spec_arr[$key][$j]["type1"] = "Secondary";
                        $j++;
                    }
                } else {
                    $j=0;
                    foreach($spec as $sp){
                        $spec_arr[$key][$j]["type1"] = "Other";
                        $j++;
                    }
                }
                $i++;
    ...
    /*
     * @author Ravindra 06/09/2019
     * Get Country Flag By Seller Id
     */

    public function getCountryFlag($seller_id) {
    ...
    /*
     * @author Ravindra 06/09/2019
     * Get Company Register Year By Seller Id
     */

    public function getRegisterYear($seller_id) {
    ...
    public function getProductsdetailsByCategory($category_id, $start_from = 0, $limit, $min_order = 0, $min_price = 0, $max_price = 0, $sortby = 0) {
    ...
    public function getProductsdetailsByCategoryCount($category_id, $min_order=0, $min_price=0, $max_price=0) {
        //changed after applying offer to product category
        //Yogesh Pardeshi 16-08-2019 6:33 pm
        $this->db->select("P.id");
        $this->db->from("product_details P");
    ...
    function getfavProductDetails($prod_id) {
        $this->db->select($this->selectOffer . 'MAX(final_price) as max_final_price, units_name, quantity_upto, quantity_from, atz_price as mrp, discount_percentage as discount,low_stock_qty,available_quantity, tb1.id,tb1.name,s.company_name,s.location_country,s.comp_operational_city,s.year_of_register,PM.url as media_url');
        $this->db->from("product_details tb1");
        $this->db->join("offer_categories oc", "oc.category_id = tb1.category AND oc.offer_id = (SELECT oz.offer_id FROM offer_zone oz WHERE oz.status = 'ACTIVE' AND oz.offer_id = oc.offer_id limit 1)", 'left');
    ...
    function get_recommended_list() {
        $this->db->select($this->selectOffer . "PD.id,PD.name,PM.url,PC2.quantity_from,PC2.price as max_price,PP2.price as min_price,PC2.final_price as max_final_price,"
        ...
        $this->db->join("offer_categories oc", "oc.category_id = PD.category AND oc.offer_id = (SELECT oz.offer_id FROM offer_zone oz WHERE oz.status = 'ACTIVE' AND oz.offer_id = oc.offer_id limit 1)", 'left');
        $this->db->join("offer_zone oz", " oz.offer_id = oc.offer_id", 'left');
        $this->db->join("product_media PM", "PM.id = (SELECT id FROM product_media PM1 WHERE "
    ...
    function get_recommended_products_by_category($cat_id, $pid) {
        $this->db->select($this->selectOffer . 'p.id, p.discount_percentage as discount, name, price, final_price, quantity_upto, units_name,atz_price as mrp');
        $this->db->from('product_details p');
        $this->db->join('product_price pp', 'p.id = pp.product_id');
        $this->db->join("categories cat", "cat.category_id = p.category", 'left');
        $this->offerJoins_p();
     ...
     * @use
     **/
    public function getProductData($productId, $offerId, $totalQty) {
        if($productId == NULL || $productId == 0){
            return;
        }
        if($offerId != 0){
            $prices = array();
            $prices['mrp'] = $this->db->select(' atz_price mrp ')
                    ->from('product_price pp')
                    ->where("pp.product_id = $productId")
                    ->where("$totalQty between quantity_from AND quantity_upto")
                    ->limit("1")
                    ->get()->result_array()[0]['mrp'];
            
            $offerData = $this->db->select('discount_value discount, offer_type')
                    ->from('offer_zone')
                    ->where("offer_id= $offerId")
                    ->limit("1")
                    ->get()->result_array()[0];
            $prices['discount'] = $this->Offer_model->offerDiscount($offerData['offer_type'], $offerData['discount']);
            return $prices;
        } else {
           $prices =  $this->db->select('concat(discount_percentage, " %") discount, atz_price mrp ')
    ...
    public function topSellingProductsData($limit = 10) {
        $this->db->limit($limit);
        $this->db->select($this->selectOffer . "PD.id,PD.name,PM.url,PC2.quantity_from,PC2.price as max_price,PP2.price as min_price,"
    ...
    public function getDiscoutedProduct($value = 5, $type = "percentage", $start = 0, $limit = 10) {
        $select = "DISTINCT(P.id) as product_id,
        P.name,
        C.categories_name,
        PM.type as media_type,
    ...
    public function getProductsUptoDiscount($value = 5, $type = "percentage", $start = 0, $limit = 10) {
        $select = "DISTINCT(P.id) as product_id,
        P.name,
        C.categories_name,
        PM.type as media_type,
    ...
    function getProductDetailsYourFav($products_id = 0) {
        ...
        $this->db->select($this->selectOffer . 'p.*,pm.url,pp.*');
        $this->db->from('product_details p');
        $this->db->join("categories cat", "cat.category_id = p.category", 'left');
        $this->offerJoins_p();
        $this->db->join('product_media pm', 'pm.product_id = p.id');
    ...
    public function getProductByCatIds_remain($cats, $limit = 30, $start_from = 0) {
        ...
        $this->db->select($this->selectOffer . $select);
        $this->db->from($this->_table . " P");
        $this->db->join($this->_tableCategory . " C", "P.category = C.categories_id");
        $this->offerJoins();
        $this->db->join($this->_tableMedia . " PM", "PM.id = (SELECT id FROM $this->_tableMedia PM1 WHERE "
    ...
    public function getSerachProductByKeywordCount($keyword) {
        //$this->output->enable_profiler(TRUE);
        $this->db->select("P.id");
        $this->db->from("$this->_table P");
        $this->offerJoins();
    ...
    public function insertInNotifyList($product_id, $user_id) {
        if (isset($product_id) && isset($user_id)) {
            ...
            if ($checkDuplicate['id'] > 0) {
                ...
                $inserted = $this->db->insert('product_notify_list', array('product_id' => $product_id,
                    'user_id' => $user_id));
                if ($inserted > 0) {
                    return true;
                }
    ...
    public function offerJoins() {
        $this->db->join("offer_categories oc", "oc.category_id = P.category AND oc.offer_id = (SELECT oz.offer_id FROM offer_zone oz WHERE oz.status = 'ACTIVE' AND oz.offer_id = oc.offer_id AND (NOW() >= oz.valid_from AND NOW()<=oz.valid_to) limit 1)", 'left');
        $this->db->join("offer_zone oz", " oz.offer_id = oc.offer_id", 'left');
    }

    /**
     * @auther Yogesh Pardeshi 30082019
     * @used to call offer join on various products for small p alias
     * */
    public function offerJoins_p() {
        $this->db->join("offer_categories oc", "oc.category_id = p.category AND oc.offer_id = (SELECT oz.offer_id FROM offer_zone oz WHERE oz.status = 'ACTIVE' AND oz.offer_id = oc.offer_id AND (NOW() >= oz.valid_from AND NOW()<=oz.valid_to) limit 1)", 'left');
        $this->db->join("offer_zone oz", " oz.offer_id = oc.offer_id", 'left');
    }
    
    public function seoUrl($title, $separator = '-')
    {
    ...
    public function getProductsByCategoryAndTerm($category_id, $start_from = 0, $limit, $min_order=0, $min_price=0, $max_price=0, $sortby=0,$term=0) {
        //changed after applying offer to product category
        //Yogesh Pardeshi 16-08-2019 6:33 pm
        $this->db->select(
                $this->selectOffer
    ...
    public function getProductsByCategoryAndTermCount($category_id, $min_order=0, $min_price=0, $max_price=0, $term=0) {
        //changed after applying offer to product category
        //Yogesh Pardeshi 16-08-2019 6:33 pm
        $this->db->select("P.id");
        $this->db->join("categories_description C", "P.category = C.categories_id", 'left');
    ...
    public function getSupplierInfo($supplierId) {
        $this->db->select('s.company_name,address1,registration_state,comp_operational_addr,comp_operational_city,comp_operational_state,comp_operational_region,comp_operational_zip_code,ct.name business_type,first_name,last_name, iso3 country_name,iso, u.email, u.phone, c.name as cntry');
        $this->db->from('seller_info s');
        $this->db->join('users u', 's.user_id = u.id');
        $this->db->join('country c', 'u.country = c.id');
        $this->db->join('seller_company_details sc', 'sc.user_id = s.user_id');
        $this->db->join('company_types ct', 'ct.id = s.company_type');
        $this->db->where('s.user_id ', $supplierId);
        $this->db->limit(1);
