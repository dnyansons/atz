...
class Product_model extends CI_Model
{
    ...
    * Functions userd with server side datatables
    ...
    public function get_datatables($from, $to, $seller = 0, $category = "", $name = "", $publish_status = "initiated") {
        $this->_get_datatables_query($from, $to, $seller, $category, $name, $publish_status);
    ...
    public function count_filtered($from, $to, $seller = 0, $category = "", $name = "", $publish_status = "approved") {
    ...
    private function _get_datatables_query($from, $to, $seller = 0, $category = "", $name = "", $publish_status)
    {
        //secho $publish_status;
        ...
        if ($from != '' && $to != '' || $from != NULL) { // To process our custom input parameter
            $this->db->where('approved_on BETWEEN "' . date('Y-m-d', strtotime($from)) . '" and "' . date('Y-m-d', strtotime($to)) . '"');
        }
    ...
    public function getProductsByCategoryData_app($category_id, $start_from = 0, $limit = 15) { //api // api
        ...
        $this->db->join("units as units", "PC1.unit=units.units_id", "left");
        $this->db->join("seller_company_details as scd", "scd.user_id=P.seller", "left");
        $this->db->where_in('P.category', $cat_id);
        $this->db->where('P.publish_status','approved');
        $query = $this->db->get();
        $result = $query->result_array();
	...
	function get_products($cat_id)
	{
        ...
        public function getProductsdetailsByCategory($category_id, $start_from = 0, $limit = 30) {
            $this->db->limit($limit, $start_from);
	...
	{
		$this->db->select('MAX(final_price) as max_final_price,MIN(final_price) as min_final_price, units_name, quantity_upto, quantity_from');
	...
	{
		$this->db->select("PD.id,PD.name,PM.url,PC2.quantity_from,PC2.price as max_price,PP2.price as min_price,PC2.final_price as max_final_price,"
	...
	function get_recommended_products_by_category($cat_id,$pid)
	{
		...
		$this->db->select('p.id, p.discount_percentage as discount, name, price, final_price, quantity_upto, units_name,atz_price as mrp');
	...
	{
		$this->db->limit($limit);
		$this->db->select("PD.id,PD.name,PM.url,PC2.quantity_from,PC2.price as max_price,PP2.price as min_price,"
	...
	*/
	...
	public function getSellerInformation($seller_id)
	{
		...
		$this->db->from('seller_info s');
		$this->db->join('users u','s.user_id = u.id');
		$this->db->join('address_book ad','ad.user_id = u.id'); //change Here add this table line
		$this->db->join('country c','u.country = c.id');
		$this->db->join('company_types ct','ct.id = s.company_type');
		$this->db->where('s.user_id',$seller_id);
	...
	$this->db->select('id,seller_id,user_id,product_id,product_total_quantity,product_name,product_image,supplierDetails,specifications');
    ...
    function get_filterdProducts($category_id,$min_order,$min_price,$max_price)
    {
    ...
    public function readSingleProduct($productId)
    {
        ...
        if($productId != null && !empty($productId) && $productId != 0)
        {
            $product_deatils = $this->db->select('u.status as active_status,u.email, phone, u.id seller_id, CONCAT(first_name, " ", last_name) seller_name, pd.id, name, keywords, seller, categories_name,  description, discount_percentage,
                                           discount_percentage, weight, width, height, length, provide_order_at_buyer_place, price_type,
